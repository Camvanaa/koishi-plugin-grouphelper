 ...
 * 举报审核 Prompt —— 唯一来源
 *
 * 这两段 Prompt 此前在 report.module.ts 与 settings.manager.ts 各存一份，
 * 且已经分叉：settings 那份完全没有描述 reporterPenalty 字段。
 * 由于 getDefaultPrompt() 会优先取 settings 中的值，而 DEFAULT_SETTINGS 恒有值，
 * 实际生效的一直是缺字段的那份 —— AI 从未被要求返回 reporterPenalty，
 * 「限制滥用举报者」功能在默认配置下从来没有工作过。
 *
 * 因此这里统一导出，settings 与模块都从本文件取值，让分叉在结构上不可能发生。
 */

/** 不带上下文的审核 Prompt，占位符：{content} */
...
/** 带群聊上下文的审核 Prompt，占位符：{context}、{content} */
