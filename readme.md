...
- `nickname <@用户> [昵称]` 设置用户群昵称，昵称留空则为清除
...
- `clearlog` 清理日志文件（默认权限4级）：
  - `-d <天数>` 保留最近几天的日志，默认7天
  - `-a` 清理所有日志

### 📊 命令日志统计

- `cmdlogs.cehck` 查看命令执行日志：
  - `-l <数量>` 显示条数，默认10条
  - `-u <用户ID>` 筛选特定用户
  - `-c <命令>` 筛选特定命令
  - `-f` 只显示失败的命令
...
- `cmdlogs.clear` 清除命令日志
- `cmdlogs.export` 导出命令日志：
  - `-d <天数>` 导出最近N天的日志，默认7天
...
- `quit-group <群号>` 让bot退出指定群聊（权限等级4）
- `ban-list` 查看禁言名单
